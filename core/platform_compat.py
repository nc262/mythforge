...
def suppress_console_windows() -> None:
    ...
    Odysseus runs headless under pm2 (the parent has no console of its own). On
    Windows, when a *console* subprocess (git, tailscale, ``python -c`` cache
    probes, …) is launched from a parent that has no console, the OS allocates a
    brand-new **visible** console window for the child. Many call sites spawn such
    probes on timers (model discovery, HuggingFace cache checks, repo helpers),
    so the desktop fills with windows that rapidly open and close.

    Rather than thread a creation flag through ~30 call sites (and every future
    one), patch :class:`subprocess.Popen` once at startup so that — on Windows
    only — children default to ``CREATE_NO_WINDOW`` UNLESS the caller already
    chose a console mode (``CREATE_NEW_CONSOLE`` / ``DETACHED_PROCESS`` /
    ``CREATE_NO_WINDOW``). Explicit detach behaviour (see
    :func:`detached_popen_kwargs`) is therefore preserved untouched. Idempotent;
    a no-op on POSIX.
    """
    if not IS_WINDOWS:
        return
    ...
    console_mode_flags = CREATE_NO_WINDOW | CREATE_NEW_CONSOLE | DETACHED_PROCESS

    orig_init = subprocess.Popen.__init__
    if getattr(orig_init, "_odysseus_no_window", False):
        return  # already patched

    def _init(self, *args, **kwargs):
        flags = kwargs.get("creationflags", 0)
        if not (flags & console_mode_flags):
            kwargs["creationflags"] = flags | CREATE_NO_WINDOW
        return orig_init(self, *args, **kwargs)

    _init._odysseus_no_window = True
