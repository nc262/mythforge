...
def _build_ollama_payload(
    ...
    if tools:
        payload["tools"] = tools
    if json_mode:
        # Grammar-constrain the decode to valid JSON. Ollama enforces this at
        # the token level, so the model cannot emit prose around the object.
        payload["format"] = "json"
    return payload
...
async def llm_call_async(
    ...
    ``json_mode`` opt-in constrains the decode to a single valid JSON value
    (Ollama ``format:"json"`` / OpenAI ``response_format``). Use it for
    structured-extraction callers so a chatty model can't wrap the object in
    prose. Anthropic has no equivalent here and is left unconstrained.

    ``num_ctx`` (Ollama only) caps the context window for this call instead of
    using the model's full advertised size. Short-prompt callers (extraction)
    should pass a small value so Ollama doesn't allocate a huge KV cache for a
    128k window it won't use — a large speedup on load and inference.
    """
    provider = _detect_provider(url)
    messages_copy = _sanitize_llm_messages(messages)
    ...
    if provider == "anthropic":
        ...
        payload = _build_ollama_payload(
            model, messages_copy, temperature, max_tokens,
            stream=False, num_ctx=(num_ctx if num_ctx else get_context_length(url, model)),
            json_mode=json_mode,
        )
    else:
        target_url = url
