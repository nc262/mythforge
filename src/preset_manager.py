...
"""
    ...
    def save_user_template(self, template: dict) -> bool:
        ...
        if existing is not None:
            prev = templates[existing]
            merged = {**prev, **template}
            # Preserve Character Studio fields when an incoming save omits them or
            # sends them empty. The persona auto-save in presets.js only carries
            # id/name/system_prompt/temperature/max_tokens, so without this it
            # would silently wipe a Studio persona's avatar/reference/appearance.
            for k in ("avatar", "character_name", "appearance", "system_prompt",
                      "personality", "world", "setting", "relationship", "scene", "world_id"):
                if not template.get(k) and prev.get(k):
                    merged[k] = prev[k]
            templates[existing] = merged
        else:
