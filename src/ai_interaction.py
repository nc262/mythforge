    ...
    try:
        ...
        for ep in endpoints:
            ...
            if provider == "anthropic":
                ...
                cached_model_ids = []
                try:
                    cached_model_ids = json.loads(ep.cached_models or "[]")
                except Exception:
                    cached_model_ids = []
                if not isinstance(cached_model_ids, list):
                    cached_model_ids = []
                cached_model_ids = [
                    str(m).strip()
                    for m in cached_model_ids
                ...
                try:
                    ...
                    if models_url:
                        ...
                        model_ids = cached_model_ids
                except Exception:
                    model_ids = []

                # Local image bridges and llama.cpp-style servers can be
                # temporarily busy or slow to answer /models. The endpoint row's
                # cache is populated by Settings/Cookbook discovery, so keep it
                # as a valid fallback instead of treating a probe miss as "model
                # does not exist".
                seen_model_ids = set()
                model_ids = [
                    str(mid).strip()
                    for mid in list(model_ids or []) + cached_model_ids
                    if isinstance(mid, str)
                    and str(mid).strip()
                    and not (str(mid).strip() in seen_model_ids or seen_model_ids.add(str(mid).strip()))
                ]
...
async def do_generate_image(content: str, session_id: Optional[str] = None, owner: Optional[str] = None, character: Optional[str] = None, steps: Optional[int] = None, cfg: Optional[float] = None, regions: Optional[list] = None, matte: bool = False) -> Dict:
    ...
    # Character-consistency: forward the persona key to the local bridge so it
    # conditions on that character's reference photos (IP-Adapter). Local only —
    ...
    if is_local_diffusion:
        ...
        if matte:
            payload["matte"] = True   # World Compiler item art: return a cut-out (alpha)
    ...
    try:
        ...
        logger.exception("Image generation request failed: endpoint=%s model=%s", images_url, model_id)
