...
function getFolderNames() {
  const names = new Set();
  // "Adventures" is the Character Studio's reserved folder — it lives in the
  // game, not the sidebar, so it never appears as a folder group or move target.
  sessions.forEach(s => { if (s.folder && s.folder !== 'Adventures') names.add(s.folder); });
...
function _renderSessionListImpl() {
  ...
  const savedOrder = Storage.get('session-order');
  // Adventures (Character Studio "dm-*" Game-Master sessions) live inside the
  // game — they're resumed from the Studio, not this companion/chat list — so
  // keep them out of the sidebar. The studio's char→session map is the source
  // of truth for which session ids are adventures. ponytail: read its
  // localStorage directly rather than coupling the two modules.
  let _advSids = new Set();
  try {
    const m = JSON.parse(localStorage.getItem('studio-char-sessions') || '{}');
    for (const [cid, sid] of Object.entries(m)) if (String(cid).startsWith('dm-')) _advSids.add(String(sid));
  } catch (e) {}
  let orderedSessions = sessions.filter(s => !s.archived && s.folder !== 'Assistant' && s.folder !== 'Adventures' && !_isIncognitoSession(s.id) && !_advSids.has(String(s.id)) && (s.name || '').trim() !== 'Nobody' && (s.name || '').trim() !== 'Incognito');
...
export async function loadSessions() {
  try {
    ...
    // Game mode boots to the title screen — never auto-restore an old chat on
    // page load (refresh used to land in the legacy chat surface). Sessions
    // still open when explicitly chosen (Continue, companion picker, party).
    const _gameHome = document.body.classList.contains('game-mode') &&
      document.body.classList.contains('home-landing') && !currentSessionId && !hasPendingChat;
    if (_gameHome) targetId = null;
    ...
    if (!targetId && !hasPendingChat) {
      ...
      if (activeSessions.length === 0 && !_autoCreateInProgress && !_gameHome) {
