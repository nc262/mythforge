...
export async function _syncFromServer() {
  if (_cookbookForbidden) return false;   // non-admin (e.g. a game player) — stop polling a forbidden endpoint
  try {
    const res = await fetch('/api/cookbook/state', { credentials: 'same-origin' });
    if (res.status === 403) { _cookbookForbidden = true; return false; }
...
// Non-admin players don't have cookbook access — one 403 means every poll
// will 403, so stop the monitor instead of spamming the console forever.
let _cookbookForbidden = false;

async function _pollBackgroundStatus() {
  if (_cookbookForbidden) return;
  try {
    ...
    if (res.status === 403) { _cookbookForbidden = true; _stopBackgroundMonitor(); return; }
    if (!res.ok) return;
    const data = await res.json();
