...
// Global lookup so the chat renderer can show a persona's avatar next to their
// messages, matched by display name or character_name (case-insensitive).
// Closes over the live `userTemplates` cache, so it reflects the latest load.
window.getPersonaAvatar = function (name) {
  if (!name) return null;
  const key = String(name).trim().toLowerCase();
  const t = (userTemplates || []).find(
    (x) => (x.name || '').trim().toLowerCase() === key
        || (x.character_name || '').trim().toLowerCase() === key);
  return t && t.avatar ? t.avatar : null;
};
...
// ── Companion picker ────────────────────────────────────────────────────────
// The game-styled launcher behind "Talk with a Companion": pick a saved persona
// (1:1 chat), start a group, forge a new one, or just chat with no persona.
// Activating a persona reuses the same /api/presets/custom path the modal uses,
// so all the downstream wiring (preset_id=custom, avatar lookup) just works.
async function _activateTemplate(tmpl) {
  const max_tokens = (tmpl.max_tokens && tmpl.max_tokens <= 8192) ? tmpl.max_tokens : 0;
  const config = {
    name: tmpl.character_name || tmpl.name || '',
    enabled: true,
    temperature: Math.max(0, Math.min(2, tmpl.temperature != null ? tmpl.temperature : 1.0)),
    max_tokens,
  ...
  try {
    const r = await fetch(`${API_BASE}/api/presets/custom`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(config),
    });
    const res = await r.json();
    if (res && res.success) {
      presets.custom = { ...presets.custom, ...config, character_name: config.name };
      selectedPreset = 'custom';
      // A persona chat must not also run group fan-out (it would hijack sends).
      try {
        if (window.groupModule && window.groupModule.isActive()) {
          window.groupModule.stopGroup();
          if (window._syncGroupIndicator) window._syncGroupIndicator(false);
...
// Leave the landing for a fresh chat in the companion workspace: new session,
// reveal the sidebar/composer (drop home-landing), focus the input. Starting a
// new chat DEACTIVATES any persona, so when a companion is chosen we activate it
// on the next tick — after the reset — or it'd be wiped out from under us.
function _enterCompanionChat(tmpl) {
  const nc = document.getElementById('sidebar-new-chat-btn') || document.getElementById('rail-new-session');
  if (nc) nc.click();                                // fresh session (clears any open chat + persona)
  document.body.classList.remove('home-landing');    // reveal sidebar + composer
  const focus = () => { const m = document.getElementById('message'); if (m) { try { m.focus(); } catch (e) {} } };
  if (tmpl) setTimeout(() => { _activateTemplate(tmpl).then(focus); }, 0);
  else focus();
}

export async function openCompanionPicker() {
  await loadUserTemplates();
  let ov = document.getElementById('companion-picker-overlay');
  if (!ov) { ov = document.createElement('div'); ov.id = 'companion-picker-overlay'; document.body.appendChild(ov); }
  const esc = (s) => String(s == null ? '' : s).replace(/[&<>"]/g, c => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;' }[c]));
  // Companions only: custom personas (user-*) and world cast (wc-*). Game
  // Masters / adventures (dm-*) belong to the game surface, not companion chat.
  const cards = (userTemplates || [])
    .filter(t => (t.character_name || t.name) && !String(t.id || '').startsWith('dm-'))
    .map(t => {
    const nm = t.character_name || t.name;
    const av = t.avatar
      ? `<img src="${esc(t.avatar)}" alt="" loading="lazy">`
      : `<span class="cp-initial">${esc((nm || '?').trim().charAt(0).toUpperCase())}</span>`;
    return `<button type="button" class="cp-card" data-name="${esc(t.name || nm)}">
      <span class="cp-avatar">${av}</span><span class="cp-name">${esc(nm)}</span></button>`;
  }).join('');
  ov.innerHTML = `<div class="cp-sheet" role="dialog" aria-modal="true" aria-label="Choose a companion">
      <div class="cp-bar"><h2>Talk with a Companion</h2><button class="cp-close" id="cp-close" type="button" aria-label="Close">✕</button></div>
      <div class="cp-grid">
        ${cards || '<p class="cp-empty">No companions yet — forge your first one below.</p>'}
        <button type="button" class="cp-card cp-action" id="cp-new"><span class="cp-avatar cp-plus">＋</span><span class="cp-name">New companion</span></button>
        <button type="button" class="cp-card cp-action" id="cp-group"><span class="cp-avatar cp-plus">⚇</span><span class="cp-name">Group chat</span></button>
      </div>
      <button type="button" class="cp-plain" id="cp-plain">Just chat — no persona</button>
    </div>`;
  ...
  document.addEventListener('keydown', function onEsc(e) {
    if (e.key === 'Escape' && ov.style.display !== 'none') { close(); document.removeEventListener('keydown', onEsc); }
  });
  ov.querySelectorAll('.cp-card[data-name]').forEach(btn => btn.addEventListener('click', () => {
    const t = userTemplates.find(x => (x.name || x.character_name) === btn.dataset.name);
    close();
    _enterCompanionChat(t);   // fresh session, then activate the chosen companion
  }));
  document.getElementById('cp-new').addEventListener('click', () => {
    close();
    // The studio's forge (portrait gen + guided fields + companion/GM toggle)
    // is the one creation tool; the old persona modal is the fallback.
    if (window.characterStudio && window.characterStudio.open) window.characterStudio.open(true);
    else openCustomPresetModal();
  });
  document.getElementById('cp-group').addEventListener('click', () => {
    close();
    openCustomPresetModal();
    // Jump straight to the persona modal's existing Group tab.
    setTimeout(() => { const gt = document.querySelector('.preset-tab[data-chartab="group"]'); if (gt) gt.click(); }, 0);
  });
  document.getElementById('cp-plain').addEventListener('click', () => { close(); deactivateCharacter(); _enterCompanionChat(); });
}
window.openCompanionPicker = openCompanionPicker;
