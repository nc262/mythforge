...
export function hideWelcomeScreen() {
  ...
  // Leaving the landing: bring back the rail/sidebar/composer chrome.
...
export function showWelcomeScreen() {
  ...
  // Back to the clean landing: the two choices only, chrome hidden.
...
export function addMessage(role, content, modelName, metadata) {
  try {
    ...
    // Persona avatar: show the character's face next to their name (matched by
    // name via the presets cache). Additive + gated, so non-persona messages are
    // untouched and a missing avatar just falls back to the dot.
    if (role !== 'user' && metadata?.character_name && !isSlash && !isCompacted) {
      try {
        const _av = window.getPersonaAvatar?.(metadata.character_name);
        if (_av) {
          const _aimg = document.createElement('img');
          _aimg.className = 'role-persona-avatar';
          _aimg.src = _av;
          _aimg.alt = metadata.character_name;
          _aimg.loading = 'lazy';
          r.insertBefore(_aimg, r.firstChild);
          r.classList.add('has-avatar');
        }
      } catch (_e) { /* avatar is cosmetic; never block rendering */ }
    }
    if (role !== 'user') {
