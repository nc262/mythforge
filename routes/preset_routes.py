...
class UserTemplateRequest(BaseModel):
    ...
    # Character Studio fields (optional): the persona's avatar/image-reference URL,
    # the character key used for IP-Adapter reference folders, and the appearance
    # anchor. save_user_template merge-preserves these when a save omits them, so
    # the lightweight persona auto-save in presets.js can't wipe a Studio persona.
    avatar: str = Field("", max_length=2000)
    character_name: str = Field("", max_length=100)
    appearance: str = Field("", max_length=400)
...
def setup_preset_routes(preset_manager) -> APIRouter:
    ...
    async def get_user_templates(request: Request) -> List[Dict]:
        # Characters and adventures are per-player. Ownerless templates predate
        # ownership and stay visible to everyone on this box (single-household
        # legacy); new saves always carry an owner.
        user = get_current_user(request)
        return [t for t in preset_manager.get_user_templates()
                if not t.get("owner") or t.get("owner") == user]
    ...
    async def delete_user_template(template_id: str, request: Request) -> Dict[str, Any]:
        # Owners delete their own characters; legacy ownerless ones need admin.
        user = get_current_user(request)
        t = next((x for x in preset_manager.get_user_templates() if x.get("id") == template_id), None)
        if t is None:
            return {"success": False, "message": "No such template"}
        if t.get("owner"):
            if t["owner"] != user:
