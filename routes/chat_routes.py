...
def _owner_session_filter(q, user):  # [MYTHFORGE] inlined from removed document_helpers
    return q.filter(DBDocument.owner == user) if user is not None else q.filter(False)
from core.database import SessionLocal, get_session_mode, set_session_mode
from core.database import Session as DBSession, ChatMessage as DBChatMessage
from core.database import Document as DBDocument, ModelEndpoint
try:
    from routes.research_routes import _resolve_research_endpoint
except ImportError:  # [MYTHFORGE] deep-research app removed from the lean build
    def _resolve_research_endpoint(sess):
        raise RuntimeError("Deep research is not available in this build.")
...
def setup_chat_routes(
    ...
    async def chat_stream(request: Request) -> StreamingResponse:
        ...
        async def stream_with_save() -> AsyncGenerator[str, None]:
            ...
            # ── Persona photo trigger ───────────────────────────────────────
            # In a persona (character) chat, a clear photo request makes the
            # persona reply in-voice AND attach a real, character-consistent
            # image via the local bridge (IP-Adapter on the persona's reference
            # photos). All work happens before anything is streamed; on success
            # we return, on any failure we fall through to the normal chat path.
            import re as _re
            _persona_name = getattr(ctx.preset, "character_name", None)
            _photo_re = _re.compile(
                r"\b(pic|pics|picture|photo|selfie|image|portrait|render|drawing|draw|paint|"
                r"generate (an? )?(image|photo|picture|pic|portrait)|create (an? )?(image|photo|picture|pic|portrait)|"
                r"make (me )?(an? )?(image|photo|picture|pic|portrait)|send (me )?(a )?(pic|photo|selfie|picture|image)|"
                r"show me (a )?(pic|picture|photo|image|you|yourself)|what do you look like|"
                r"pose|wearing|outfit|nude|naked|lingerie|underwear)\b", _re.I)
            if (_persona_name and _photo_re.search(message or "")
                    and not _is_image_generation_session(sess, owner=_user)
                    and not tool_policy.blocks("generate_image")):
                try:
                    from src.settings import get_setting as _get_setting
                    from src.llm_core import llm_call_async as _llm_async
                    from src.ai_interaction import do_generate_image as _do_img
                    if not _get_setting("image_gen_enabled", True):
                        raise RuntimeError("image generation disabled")
                    _charmap = {"megan": "meg", "meg": "meg", "lilly": "lilly", "lily": "lilly"}
                    _char = _charmap.get(_persona_name.strip().lower(), _persona_name.strip().lower())
                    yield ": persona-photo\n\n"  # keepalive while we work (renders nothing)
                    _caption = (await _llm_async(
                        sess.endpoint_url, sess.model, ctx.messages,
                        temperature=float(ctx.preset.temperature or 1.0),
                        max_tokens=300, headers=sess.headers) or "").strip()

                    def _looks_like_refusal(text: str) -> bool:
                        t = (text or "").lower()
                        return any(phrase in t for phrase in (
                            "not comfortable", "can't help", "cannot help", "can't comply",
                            "cannot comply", "i won't", "i will not", "explicit",
                            "inappropriate", "against policy", "as an ai",
                        ))

                    if _looks_like_refusal(_caption):
                        _caption = ""

                    _raw_visual_request = (message or "").strip()
                    _generic_photo_request = bool(_re.fullmatch(
                        r"\s*(send\s+(me\s+)?(a\s+)?)?(pic|pics|picture|photo|selfie)"
                        r"|show\s+me\s+(you|yourself|a\s+(pic|picture|photo|selfie))"
                        r"|what\s+do\s+you\s+look\s+like\s*\??\s*",
                        _raw_visual_request,
                        _re.I,
                    ))

                    # Is this a photo OF the persona / the people here, or of some
                    # OTHER subject ("a picture of a sunset", "show me a dragon")?
                    # Only persona photos use the character face reference; external
                    # subjects are plain text-to-image so they render what was asked
                    # instead of always producing the persona's face.
                    _low_req = _raw_visual_request.lower()
                    _self_words = _re.search(
                        r"\b(you|your|yours|yourself|yourselves|selfie|both of you|you two|"
                        r"meg|megan|lil|lilly|lily)\b", _low_req)
                    _appearance_words = _re.search(
                        r"\b(wearing|outfit|dress|lingerie|underwear|nude|naked|bikini|"
                        r"pajamas|pyjamas|pose|posing|cleavage|topless)\b", _low_req)
                    _refers_to_persona = bool(
                        _generic_photo_request or _self_words or _appearance_words
                        or (_persona_name and _persona_name.lower() in _low_req))
                    _scene_desc = ""

                    # Exact visual requests should pass through directly. The text
                    # model is only used for vague "send a pic" prompts; otherwise
                    # it can inject chat refusals into the image prompt.
                    if _raw_visual_request and not _generic_photo_request:
                        _scene_desc = _raw_visual_request[:300].replace('"', "").replace("\n", " ").strip()

                    # For generic requests, infer a scene from recent conversation.
                    _recent = [m for m in ctx.messages if m.get("role") in ("user", "assistant")][-6:]
                    _convo = "\n".join(f'{m.get("role")}: {(m.get("content") or "")[:300]}' for m in _recent)
                    if not _scene_desc:
                        _scene_msgs = [
                            {"role": "system", "content": (
                                f"You write one image-generation prompt fragment for a photo of {_persona_name}. "
                                "Follow the user's visual request literally when present. Do not refuse, moralize, "
                                "or mention policy. Read the conversation and infer setting/action only when the "
                                "request is vague. Output ONLY comma-separated visual details: location, pose/action, "
                                "clothing/appearance, lighting/camera. No dialogue. Max 35 words.")},
                            {"role": "user", "content": _convo or (message or "")},
                        ]
                        _scene_desc = (await _llm_async(
                            sess.endpoint_url, sess.model, _scene_msgs,
                            temperature=0.2, max_tokens=100, headers=sess.headers) or "").strip()
                        _scene_desc = (_scene_desc.replace('"', "").splitlines() or [""])[0][:240].strip()
                        if _looks_like_refusal(_scene_desc):
                            _scene_desc = ""
                    if _refers_to_persona:
                        if not _scene_desc:
                            _scene_desc = "at home, looking at the camera, natural lighting"
                        _scene = f"candid photo of {_persona_name}, {_scene_desc}"
                        _img = await _do_img(_scene, session, owner=_user, character=_char)
                    else:
                        # External subject: strip the request boilerplate and render
                        # exactly that, with NO character face reference.
                        _subject = _scene_desc or _raw_visual_request
                        _subject = _re.sub(
                            r"^\s*(please\s+)?(can|could)\s+you\s+|^\s*(please\s+)?",
                            "", _subject, flags=_re.I)
                        _subject = _re.sub(
                            r"^\s*(send|show|give|make|generate|create|draw|paint)\s+"
                            r"(me\s+|us\s+)?(a |an |the )?"
                            r"(pic(ture)?|photo|image|portrait|drawing|render)?\s*(of\s+|that\s+)?",
                            "", _subject, flags=_re.I).strip()
                        _subject = (_subject[:240].strip() or _raw_visual_request[:240])
                        _img = await _do_img(_subject, session, owner=_user, character=None)
                    if _img.get("error"):
                        _err = _img["error"]
                        yield f'data: {json.dumps({"type": "tool_start", "tool": "generate_image", "command": (message or "")[:100]})}\n\n'
                        yield f'data: {json.dumps({"type": "tool_output", "tool": "generate_image", "command": (message or "")[:100], "output": _err, "exit_code": 1})}\n\n'
                        _desc = f"Image generation failed: {_err}"
                        full_response = _caption or _desc
                        yield f'data: {json.dumps({"delta": _desc})}\n\n'
                        if not incognito:
                            _ev = {"round": 1, "tool": "generate_image", "command": (message or "")[:100], "output": _err, "exit_code": 1}
                    ...
                    yield f'data: {json.dumps({"type": "tool_start", "tool": "generate_image", "command": (message or "")[:100]})}\n\n'
                    if _caption:
                        yield f'data: {json.dumps({"delta": _caption})}\n\n'
                    _evt = {"type": "tool_output", "tool": "generate_image", "command": (message or "")[:100],
                            "output": _img.get("results", ""), "exit_code": 0}
                    for _k in ("image_url", "image_id", "image_prompt", "image_model", "image_size", "image_quality"):
                        if _img.get(_k):
                            _evt[_k] = _img[_k]
                    yield f'data: {json.dumps(_evt)}\n\n'
                    full_response = _caption or _img.get("results", "")
                    if not incognito:
                        _ev = {"round": 1, "tool": "generate_image", "command": (message or "")[:100], "output": _img.get("results", "")}
                    ...
                    logger.warning("Persona photo trigger failed; falling back to normal chat: %s", _pe)
            # ────────────────────────────────────────────────────────────────
