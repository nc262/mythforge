...
| Feature | Original | Godot status | Pri | Deps | Cx | Missing / notes |
|---|---|---|---|---|---|---|
| Opening cinematic: four genre panoramas (dragon → holo-dragon Neonspire → suburb sunset → deep space), Ken Burns + crossfades + letterbox, stars collapse into the breathing logo; skippable on any input; once per launch | (new) | ✅ scenes/ui/cinematic.gd + Art.CINE_PROMPTS | P1 | SDXL | L | ✅ FULL VIDEO shipped: SVD img2vid clips assembled to assets/video/opening.ogv (13.9s 720p Theora); stills remain the cold-cache fallback |
