...
| The tile library is ~1.3 GB of `user://`, generated per install | `user://tiles` | fine on a dev box; a release either ships it or makes every player pour it | **tracked as DI-3 in [Backlog.md](Backlog.md)** — it is a distribution decision now, not a shortcut waiting to be paid. Deliberately **not** solved by shrinking tiles: see [Terrain.md](Terrain.md) |
