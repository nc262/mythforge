...
| **World-skinned vendor stock** — *UIPolish Round 3, deferred 2026-07-21* | `tables.json vendor_stock` is one fantasy table; every family's keeper sells daggers | **P2 / M.** Per-family goods tables with real stats (cyber: mono-knife/stim-patch; everyday: pocket knife/first-aid kit…), keyed like `class_reskins`. Display-only renames would desync the pack — needs real data. |
| **Real-UI click-driver playtest tool** — *user request 2026-07-18, parked for the structural sprint* | Headless harness drives `_send()` programmatically; computer-use MCP can't grab a custom exe on Windows | **P1 / M.** In-engine click-driver: run the real scenes against the LIVE backend and synthesize actual mouse clicks through Godot's input pipeline (warp → InputEventMouseButton at each control's screen rect), screenshotting each step. Catches dead/covered/off-screen buttons the send-level harness can't. Backend+Ollama confirmed reachable; `session.cfg` already holds a login. |
