extends Node
## Dev harness (real backend + GPU): run a full World Compiler pass end-to-end so
...
## Requires a valid session in user://session.cfg (Api loads it on _ready).

func _ready() -> void:
	# NOT test_mode — this hits the live backend, Ollama, and the GPU.
	...
	if not await Api.auth_ok():
		print("COMPILE-LIVE: not authenticated (stale cookie?) — aborting")
		get_tree().quit(1)
		return
	...
	print("=== compiling '%s' (real backend + GPU) ===" % world["name"])
	...
	for sub in counts.keys():
		...
		for f in d.get_files():
			if f.ends_with(".png"):
				...
				var tag := ""
				if img != null:
					# Report alpha presence — parts must be cut out (matted).
